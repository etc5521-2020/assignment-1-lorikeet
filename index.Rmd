...
- Does MMR Vaccination Rates Differ Across School Types?
...
```{r ida,eval=FALSE}
g <- ggplot(measles_type, aes(mmr))

g1 <- g + geom_histogram(binwidth = 1) + 
...
school_vs_overall %>%
  # convert rates to a percentage
  mutate(school_mmr_rate = percent(school_mmr_rate/100),
         school_overall_rate = percent(school_overall_rate/100),
         rate_diff = percent(rate_diff/100)) %>%
...
## Does Average Income Affect MMR Vaccination Rate?
...
measles_density_income <- measles_density %>% 
  left_join(state_income,
            by = "county_state") %>% 
  rename(percapita_income = estimate) %>% 
  filter(percapita_income != "NA")

measles_density_income %>% 
  ggplot(aes(x = percapita_income,
             y = mmr)) +
  geom_point(data = select(measles_density_income, -state),
             color = "gray") + 
...
```{r best-mmr}
measles_income_state <- measles_density_income %>%
  select(state, 
         mmr, 
         percapita_income) %>%
  group_by(state) %>% 
...
measles_income_state %>%
  # convert mmr to a percentage
  mutate(avg_mmr = percent(measles_income_state$avg_mmr/100),
  ...
  kable(caption = "The Per Capita Income of the States with the Highest MMR Vaccination Rate",
        col.names = c("States", "MMR Rate", "Per Capita Income")) %>%
  kable_styling()
```
...
```{r overall-state-plot, eval=FALSE}
