...
measles_density_income %>%
  ...
  mutate(mmr = percent(measles_density_income$mmr/100)) %>%
  # rename the column names
  rename(State = state, 
         MMR = mmr, 
         PCI = percapita_income) %>%
  arrange(desc(MMR)) %>%
...
```{r overall-state-plot, include=FALSE}
